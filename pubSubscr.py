...
class PostOffice(object):
    def __init__(self):
        self._subscriptions = {'*': []}
            # Map: topic -> list of Subscriber instances

        self._postedMessages = []
            # Messages not yet delivered

    # Extension
    def subscribeTo(self, aSubscriber, *topics):
        '''Register a Subscriber to one or more topics; '*' denotes
        all topics.'''
        if '*' in topics:
            ...
            for topic in topics:
                try:
                    ...
                    self._subscriptions[topic] = ([aSubscriber]
                                                  + self._subscriptions['*'])
        ...
        try:
            for subscriber in self._subscriptions[aMsg.topic]:
                try:
                    ...
                    self.postMessage(Message('ERROR', subscriber, '?',
