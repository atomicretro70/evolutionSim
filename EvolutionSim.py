...
class SimulationInfoFrame(tk.LabelFrame):
    ...
    def _buildGUI(self):
        ...
        e = tk.Entry(self, justify=tk.RIGHT,
                           textvariable=self._plantPop, width=6)
        e.grid(row=2, column=2, padx=2, pady=2)
        e = tk.Entry(self, justify=tk.RIGHT,
                           textvariable=self._plantEgy, width=8)
        e.grid(row=2, column=3, padx=2, pady=2)
        e = tk.Entry(self, justify=tk.RIGHT,
                           textvariable=self._cellPop,  width=6)
        e.grid(row=3, column=2, padx=2, pady=2)
        e = tk.Entry(self, justify=tk.RIGHT,
                           textvariable=self._cellEgy,  width=8)
        e.grid(row=3, column=3, padx=2, pady=2)
        e = tk.Entry(self, justify=tk.RIGHT,
                           textvariable=self._freeEgy,  width=8)
        e.grid(row=4, column=3, padx=2, pady=2)
        e = tk.Entry(self, justify=tk.RIGHT,
                           textvariable=self._totEgy,   width=8)
        e.grid(row=5, column=3, padx=2, pady=2)

        tk.Label(self, text='Population').grid(row=6, column=1)
        self._popStripView = StripChart(self, 100, 200,
                                        { 'plant':'green', 'cell':'blue' },
        ...
        self._egyStripView = StripChart(self, 100, 200,
                                        { 'plant':'green', 'cell':'blue',
                                          'free':'yellow' },
                                        borderwidth=3, relief=tk.SUNKEN)
    ...
    def updateInfo(self, tickNum, plantPop, cellPop, cellEgy, availEgy):
        ...
        self._popStripView.plotValues(plant=plantPop // 10,
                                      cell=cellPop // 10)
        self._egyStripView.plotValues(plant=plantEgy // 100,
                                      cell=cellEgy // 100)

class SimulationControlFrame(tk.LabelFrame):
    def __init__(self, parent, *args,
                 resetcommand=None, pauseplaycommand=None,
                 stepcommand=None, **opts):
    ...
    def _buildGUI(self, resetcommand, pauseplaycommand, stepcommand):
        ...
        tk.Button(self, text='Reset', compound=tk.LEFT,
                  image=self._resetImg, width=100,
                  command=resetcommand).grid(row=1, column=1)
        tk.Button(self, text='Pause/Play', compound=tk.LEFT,
                  image=self._pausePlayImg, width=100,
                  command=pauseplaycommand).grid( row=1, column=2)
        tk.Button(self, text='Tick Once',  compound=tk.LEFT,
                  image=self._stepImg, width=100,
                  command=stepcommand).grid( row=1, column=3)
        ...
        tk.Label(self, textvariable=self._tick,
                 font=tkFont.Font(family='Helvetica', size='12',
                 weight='bold')).grid(row=2, column=3, rowspan=2)
...
class EvolutionSimGUI(tk.Frame):
    ...
    def _buildGUI(self):
        ...
        self._paramsView.grid(row=1, column=1, rowspan=3,
                              sticky=tk.N+tk.S+tk.E+tk.W)

        self._worldView = WorldView(self, Params.ROWS, Params.COLUMNS,
                                    {'cell':('8.BLOCK','blue',270),
                                     'plant':('8.BLOCK','green',
                                              Params.MAX_PLANT_POPULATION)},
                                    borderwidth=3, relief=tk.SUNKEN )
        self._worldView.grid(row=1, column=2, rowspan=3)

        self._simControl = SimulationControlFrame(self,
                               resetcommand=self.reset,
                               pauseplaycommand=self.togglePausePlay,
                               stepcommand=self.tickSimuationOnce)
    ...
    def reset(self):
        ...
        self._world = WorldViewDriver(self._worldView,
        ...
        eve = SimpleCell(self._world, [c for c in Params.EVE_GENOME],
                         energy=Params.ENERGY_PER_PLANT_CELL,
                         row=eveRow, col=eveCol)
        self._sim.add(eve)

        # Create food
        Plant.AVAILABLE_ENERGY = ((Params.MAX_PLANT_POPULATION
                                   * Params.ENERGY_PER_PLANT_CELL)
                                  - (2 * Params.ENERGY_PER_PLANT_CELL))

        if Params.PLANT_SPECIES == 'Geometric':
            self._plant = GeometricPlant(self._world,
                                         Params.MAX_PLANT_POPULATION,
                                         Params.ENERGY_PER_PLANT_CELL,
                                         Params.RAND_GROWTH_FACTOR)
        elif Params.PLANT_SPECIES == 'Sinuous':
            self._plant = SinuousPlant(self._world,
                                       Params.MAX_PLANT_POPULATION,
                                       Params.ENERGY_PER_PLANT_CELL,
                                       Params.RAND_GROWTH_FACTOR)
    ...
    def updateAllInfoViews(self):
        ...
        cellEgy = ((Params.MAX_PLANT_POPULATION - plantPop)
                   * Params.ENERGY_PER_PLANT_CELL) - Plant.AVAILABLE_ENERGY
        self._simInfoView.updateInfo( simTime, plantPop, Cell.POPULATION,
                                      cellEgy, Plant.AVAILABLE_ENERGY )
