...
class WorldView(tk.Frame):
    ...
    def __init__(self, parent, numRows=10, numCols=10, objDefs=None, **opts):
        '''objs should be a dict Mapping:
            'name' -> (shape, color, maxNumber)'''
        ...
        self._canvas = tk.Canvas(self, height=height, width=width,
                                 background=WorldView.BACKGROUND)
        self._imgs = None
            # list of tk resource object refs - held to prevent
            # garbage collection
        self._cells      = None
            # 2D grid (numRows x numCols) of: imageId or None
        self._unusedObjs = None
            # Map: 'name' -> [ imgId ]
    ...
    def add(self, name, row, col):
        ...
        self._canvas.move(imgId, (col+1) * WorldView.CELL_SIZE,
                                 (row+1) * WorldView.CELL_SIZE)
    ...
    def moveTo(self, oldRow, oldCol, newRow, newCol):
        ...
        self._canvas.move(imgId, (newCol-oldCol) * WorldView.CELL_SIZE,
                                 (newRow-oldRow) * WorldView.CELL_SIZE)
    ...
    def _init(self):
        self._imgs = []
        self._cells = [[None for col in range(self._numCols)]
        ...
        for name,spec in self._objDefs.items():
            ...
            for ct in range(maxNumber):
                imgObj = tk.BitmapImage(data=SHAPE[shape],
                                        foreground=color,
                                        background=WorldView.BACKGROUND)
...
if __name__ == '__main__':
    top = tk.Tk()
    m = WorldView(top, 20, 20, {'cell':('8.BLOCK','blue',15)})
    m.grid()
