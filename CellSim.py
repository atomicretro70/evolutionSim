...
class World( object ):
    ...
    def __init__( self, numRows, numCols ):
        ...
        self._locations = [[None for col in range(numCols)]
    ...
    def neighborsOf(self, row, col):
        result = {}
        for direction,rowCol in self._neighborhood[row][col].items(): 
            # <- precomputed neighbor coords map: dirStr -> (nRow,nCol)
            nRow,nCol = rowCol
    ...
    def cNeighborsOf(self, row, col):
        result = {}
        for direction,rowCol in self._cneighborhood[row][col].items():
            # <- precomputed neighbor coords map: dirStr -> (nRow,nCol)
            nRow,nCol = rowCol
            result[direction] = nRow, nCol, self._locations[nRow][nCol]
        return result
    ...
    def neighborsOfIn(self, row, col, dirSet=DIRECTIONS):
        result = {}
        neighborhood = self._neighborhood[row][col]
        # <- precomputed neighbor coords map: dirStr -> (nRow,nCol)
    ...
    def neighborsOfType(self, row, col, aType):
        ...
        This version of the function uses precomputed coorediates for each
        neighbor.'''
        result = []
        for direction, rowCol in self._neighborhood[row][col].items():
            # <- precomputed neighbor coords map: dirStr -> (nRow,nCol)
    ...
    def cNeighborsOfType(self, r, c, aType):
        ...
        neighbor.'''
        result = []
        for direction, rowCol in self._cneighborhood[r][c].items():
            # <- precomputed neighbor coords map: dirStr -> (nRow,nCol)
            nRow,nCol = rowCol
            contents = self._locations[nRow][nCol]
            if isinstance(contents, aType):
    ...
    def emptyNeighbors(self, r, c):
        result = []
        for direction,rowCol in self._neighborhood[r][c].items():
            # <- precomputed neighbor coords map: dirStr -> (nRow,nCol)
            nRow,nCol = rowCol
    ...
    def newPos(self, row, col, direction, distance):
        if 0 <= distance <= 2:
            ...
            for ct in range(distance):
                ...
                nRow,nCol = ((row+dRow) % self._numRows,
                             (col+dCol) % self._numCols)
            return nRow,nCol
        else:
            pass # error
...
class Cell(WorldObject):
    POPULATION = 0
    R_tracker = {0:[0,0,0], 1:[0,0,0], 2:[0,0,0], 3:[0,0,0], 4:[0,0,0],
                 5:[0,0,0], 6:[0,0,0], 7:[0,0,0], 8:[0,0,0]}
    N_tracker = {0:[0,0,0], 1:[0,0,0], 2:[0,0,0], 3:[0,0,0], 4:[0,0,0],
                 5:[0,0,0], 6:[0,0,0], 7:[0,0,0], 8:[0,0,0]}

    def __init__(self, aWorld, generation=0, ident=None,
                               energy=0, row=None, col=None):
        self._birthTime = 0
        self._age = 0
        self._generation = generation
        self._energyStore = energy
        self._lastMove = '_'
    ...
    def _behavior_move(self, direction):
        newRow, newCol, content = self._world.relativePosNeighbor(self._r,
                                                                  self._c,
                                                                  direction)
        if content is not None:
            return False
...
if __name__ == '__main__':
    ...
    print('New', timeit.timeit('testNew( )',
                               setup='from __main__ import testNew',
                               number=1000))
    print('Old', timeit.timeit('testOld( )',
                               setup='from __main__ import testOld',
                               number=1000))
