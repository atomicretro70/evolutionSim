...
class Model(Subscriber):
    ...
    def __init__(self, ident=None):
        self._sim = None
           # The simulation this instance is a part of
           # (assigned in _addedToSimulation())

        self._ident = ident   # The ID of this instance
    ...
    def postMessage(self, topic, **payload):
        ...
        self._sim.postMessage(Message(topic, self,
                                      self._sim.simTime(), **payload))
...
class Simulation(PostOffice):
    def __init__(self):
        ...
        self._models = []   # list of Model instances currently in the sim.
        self._tickFn = []   # list of the tick methods of the models
        self._simTime = 0    # current simulation time
    ...
    def drop(self, aModel):
        ...
        Unsubscribe it from all topics.'''
    ...
    def tickAllModels_NoMessaging(self, numTicks):
        ...
        for t in range(self._simTime, self._simTime + numTicks):
            for tickFn in self._tickFn:
                tickFn(t)
        self._simTime += numTicks

    def tickAllModelsOnce_NoMessaging(self):
        '''Tick all models once (no message delivery, no except handling).'''
        for tickFn in self._tickFn:
            tickFn()
        self._simTime += 1
...
if __name__ == '__main__':
    ...
    import timeit
    print('==> ', timeit.timeit('runSim2()',
                                setup='from __main__ import runSim2',
                                number=100))
