...
class StripChart(tk.Frame):
    def __init__(self, parent, scale, historySize, trackColors, *args, **opts):
        ...
        self._trackHist = OrderedDict()
            # Map: Track Name -> list of canvas objID

        self._trackColor = trackColors
            # Map: Track Name -> color
        ...
        self._trackColor.update({'tick':'white',
                                 'tickline':'white',
                                 'ticklabel':'white'})
    ...
    def plotValues( self, **vals ):
        for trackName, trackHistory in self._trackHist.items():
            ...
            try:
                ...
                objId = self._canvas.create_line(x, y, x+1, y,
                                                 fill=color, width=3,
                                                 tags=trackName)
    ...
    def drawTick(self, text=None, **lineOpts):
        ...
        objId = self._canvas.create_line(x, y, x2, y2,
                                         fill=color, tags='tick', **lineOpts)
...
if __name__ == '__main__':
    ...
    val_C = 0
    delta = [-3, -2, -1, 0, 1, 2, 3]
        # randomly vary the values by one of these
